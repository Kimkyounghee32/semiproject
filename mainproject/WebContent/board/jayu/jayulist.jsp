...
<%
	...
	List<jayuBoardDto> list=dao.getList(start, end);
%>
<body>
	...
	<div class="row">
		...
		<caption><h4>자유게시판</h4></caption>
				...
				<%}else{
					...
					//댓글 dao
					jayuAnswerDao adao=new jayuAnswerDao();
					for(jayuBoardDto dto:list)
					{
						//각 글의 댓글 갯수를 acount에 넣어둔다
						int acount=adao.getJayuAnswerList(String.valueOf(dto.getNum())).size();
						...
						<tr align="center"></tr>
							...
							<td align="left" style="width:800px;">
								<%
									...
									String path="/mainproject/main.jsp?go=board/jayu/content.jsp?num="
...
<div class="pagenation" style="width: 800px; text-align: center;">
	...
	<%
		if(startPage>1){
			String pre="/mainproject/main.jsp?go=board/jayu/jayulist.jsp?pageNum"+(startPage-1);	
			%>
	...
	{
		String path="/mainproject/main.jsp?go=board/jayu/jayulist.jsp?pageNum="+i;
		if(i==currentPage){%>
		...
		if(endPage<totalPage){ //다음 : 다음블럭의 첫페이지가 active. 
			String next="/mainproject/main.jsp?go=board/jayu/jayulist.jsp?="+(endPage+1);
			%>
